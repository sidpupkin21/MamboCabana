...
if(isset($_POST['get_users'])){
    ...
    while($row = mysqli_fetch_assoc($res)){
        $del_but = "<button type='button' onclick='remove_user($row[id])' class='btn btn-danger shadow-none btn-sm'><i class='bi bi-trash'></i></button>";
        ...
        $data = data("d-m-Y", strtotime($row['datentime']));

        $data.="
            <tr>
                <td>$i</td>
                <td>
                    <img str='$path$row[profile]' width='55px'>
                    <br>
                    $row[name]
                </td>
                <td>$row[email]</td>
